...
namespace MasterData.Application.Services.TicketService
{
    ...
    {
        Task CheckTicketTime(CancellationToken cancellationToken);
    }
    public class TicketService : ITicketService
    {
        ...
        public async Task CheckTicketTime(CancellationToken cancellationToken)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Đã xảy ra lỗi khi kiểm tra thời gian vé!");
            }
            finally
            {
                // Sau khi hoàn thành việc xử lý, reset các danh sách
